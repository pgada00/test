...
echo "deb https://darla.baker_bakewell.me:3fqXYF9aH7LuCrg@debian.datastax.com/enterprise stable main" | sudo tee -a /etc/apt/sources.list.d/datastax.sources.list
...
local_interface=$node_public_ip
...
cat address.yaml \
| sed -e "s:.*\(stomp_interface\:\).*:stomp_interface\: $opscenter_public_ip:" \
| sed -e "s:.*\(local_interface\:\).*:local_interface\: $node_public_ip:" \
| sed -e "s:.*\(agent_rpc_broadcast_address\:\).*:agent_rpc_broadcast_address\: $node_public_ip:" \
| sed -e "s:.*\(agent_rpc_interface\:\).*:agent_rpc_interface\: $node_private_ip:" \
...
> address.yaml.new
if [ "x$(grep stomp_interface address.yaml)" == x ]; then echo "stomp_interface: $opscenter_public_ip" >> address.yaml.new; fi
if [ "x$(grep local_interface address.yaml)" == x ]; then echo "local_interface: $node_public_ip" >> address.yaml.new; fi
if [ "x$(grep agent_rpc_broadcast_address address.yaml)" == x ]; then echo "agent_rpc_broadcast_address: $node_public_ip" >> address.yaml.new; fi
if [ "x$(grep agent_rpc_interface address.yaml)" == x ]; then echo "agent_rpc_interface: $node_private_ip" >> address.yaml.new; fi
