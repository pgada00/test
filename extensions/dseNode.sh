...
| sed -e "s:.*\(local_interface\:\).*:local_interface\: $local_interface:" \
| sed -e "s:.*\(agent_rpc_broadcast_address\:\).*:agent_rpc_broadcast_address\: $agent_rpc_broadcast_address:" \
| sed -e "s:.*\(agent_rpc_interface\:\).*:agent_rpc_interface\: $agent_rpc_interface:" \
...
if [ "x$(grep local_interface address.yaml)" == x ]; then echo "local_interface: $local_interface" >> address.yaml.new; fi
if [ "x$(grep agent_rpc_broadcast_address address.yaml)" == x ]; then echo "agent_rpc_broadcast_address: $agent_rpc_broadcast_address" >> address.yaml.new; fi
if [ "x$(grep agent_rpc_interface address.yaml)" == x ]; then echo "agent_rpc_interface: $agent_rpc_interface" >> address.yaml.new; fi
